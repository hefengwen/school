  ...
  <div class="container">
    ...
    <div class="row clearfix allContent">
      ...
      <div class="row clearfix" style="height: 100%;">
        ...
        <div class="content">
          <div class="condition">
            <form id="condition" role="form" action="<c:url value="/user/majorInfo"/>" method="post">
              <div class="form-inline" style="margin: 10px;">
                <div style="float: right">
                  <a href="<c:url value="/user/view/majorInfoAdd"/>">
                  <img src="<c:url value="/image/addmajor.png"/>" /><b> 新增专业</b></a>
                </div>
          ...
          <div class="info">
            <table class="table table-striped table-hover" style="text-align: center;">
              ...
              <tbody>
                <c:forEach var="major" items="${result.majorList }" varStatus="status">
                  <tr>
                    <td>${(result.curPage-1)*result.pageCount+(status.index+1) }</td>
                    <td>${major.name }</td>
                    <td><fmt:formatDate value="${major.createTime }" pattern="yyyy-MM-dd HH:mm" /></td>
                    <td><a href="<c:url value="/user/majorInfoEditView/${major.majorId }"/>">编辑</a>
                      <a href="javascript:deleteMajor(${major.majorId });" />删除</a></td>
                  </tr>
                </c:forEach>
              </tbody>
            </table>
...
<script type="text/javascript">
	...
	function deleteMajor(id) {
		...
		if (!ret)
			return false;
		$.ajax({
			url : "${basePath}/user/majorInfoDelete/" + id,
			type : "POST",
