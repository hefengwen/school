  ...
  <div class="container">
    ...
    <div class="row clearfix allContent">
      <div class="row clearfix" style="height: 100%;">
        ...
        <div class="content">
          <div class="condition">
            <form id="condition" role="form" action="<c:url value="/user/courseInfo"/>" method="post">
              <div class="form-inline" style="margin:10px;">
                <div style="float:right">
                <a href="<c:url value="/user/courseInfoAddView"/>">
                  <img src="<c:url value="/image/addcourse.png"/>" /> <b>新增课程</b></a>
                </div>
              ...
              <tbody>
                  <c:forEach var="course" items="${result.courseList }" varStatus="status">
  					<tr>
                            ...
                            <td><a href="<c:url value="/study/courseInfo/${course.courseId }"/>"  target="_blank">${course.name }</a></td>
                            <td><a href="<c:url value="/user/courseBookView/${course.book.bookId }"/>" target="_blank">
                              ${fn:split(course.book.name,".")[0] }</a>
                             </td>
  						<td>
                              <fmt:formatDate value="${course.createTime }" pattern="yyyy-MM-dd HH:mm" />
  						</td>
                        <td>
                            <a href="<c:url value="/user/courseInfoEditView/${course.courseId }"/>">编辑</a>
                            <a href="<c:url value="/user/courseBookAddView/${course.courseId }"/>">教材</a>
  							<a href="javascript:deleteCourse(${course.courseId });" />删除</a>
                        </td>
  					</tr>
  				</c:forEach>
              </tbody>
...
<script type="text/javascript">
	...
	function deleteCourse(id) {
		...
		if(!ret)
			return false;
		$.ajax({
			url : "${basePath}/user/courseInfoDelete/" + id,
			type : "POST",
