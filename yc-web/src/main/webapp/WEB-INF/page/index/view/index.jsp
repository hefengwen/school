                  ...
                  <c:forEach var="resource" items="${books.resourceList }" varStatus="status">
                    <tr>
                      ...
                      <td style="text-align: left;"><img style="width: 32px; height: 32px;"
                        src="<c:url value="/image/filetype/${fn:toLowerCase(resource.suffix) }.png"/>" />
                        <span> <strong>&nbsp;&nbsp;<a
                            href="${basePath }/download/resourceInfo/${resource.resourceId}"
                            target="_blank">${resource.name }</a></strong><br /> <small>专业：<span
                        ...
                        </small> <small>评分：<span style="color: red">${resource.score }</span>分&nbsp;&nbsp;
                        </small> <span class="resourceStar" data-score="${resource.score }"></span> </small><br />
                          <small>资源简介： <c:if test="${fn:length(resource.note) > 20 }">
                          ${fn:substring(resource.note,0,10) }...
                      </c:if> <c:if test="${fn:length(resource.note) <= 20 }">
                          ${resource.note }
                      </c:if></small>
                      </span></td>
        ...
        <div class="indexRight">
          <ul class="list-unstyled rightUl" style="min-height: 262px; height: auto;">
            <li class="indexTitle"><img style="width: 32px; height: 32px;"
              src="<c:url value="/image/notice.png"/>" /><b>&nbsp;&nbsp;课程公告</b><a
              style="float: right;" href="<c:url value="/user/courseNoticeList"/>" target="_blank">更多>></a></li>
            <table class="table" style="text-align: center; width: 98%; margin: auto;">
              <tbody>
                <c:forEach var="notice" items="${courseNotices.noticeList }" varStatus="status">
                  <tr>
                    <td style="width: 65%;"><a
                      href="<c:url value="/user/noticeDetail/${notice.noticeId }"/>" target="_blank">
                        <c:if test="${fn:length(notice.title) > 10 }">
                                ${fn:substring(notice.title,0,10) }...
                            </c:if> <c:if test="${fn:length(notice.title) <= 10 }">
                                ${notice.title }
                            </c:if>
                    </a></td>
